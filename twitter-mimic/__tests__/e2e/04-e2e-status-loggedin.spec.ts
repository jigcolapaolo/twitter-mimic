...
test.describe("Status Page", () => {
  test("should run multiple assertions and be able to work the status page flow correctly", async ({
    ...
    await test.step("Verify comment button is working correctly", async () => {
        ...
        const newComment = page.locator("article:has(p:has-text('Test comentario 1'))");

        await expect(newComment).toBeVisible();
    })

    await test.step("Verify comment menu is working correctly", async () => {
        const commentMenuBtn = page.locator("div[aria-label='CommentMenuDiv'] button[aria-label='CommentMenuBtn']");
        ...
        const deleteCommentButton = page.getByRole("button", { name: "Eliminar" });
    ...
    await test.step("Verify edit comment button is working correctly", async () => {
        ...
        const commentContent = page.getByPlaceholder("Escribe un comentario").nth(1);
        ...
        const newComment = page.locator("article:has(p:has-text('Test comentario editado'))");
